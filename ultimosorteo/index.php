...
$json = file_get_contents("https://juega.loto.com.ni/websales/?action=gamesdata&gamename=El%20Acumulado%20Loto");
$data = json_decode($json, true)["El Acumulado Loto"]["Last two draws"]["draws"]["Last Draw"];

$drawnumber = $data["drawnumber"];
$result = implode($data["result"]);
...
$(function() {
	...
	var numero = ("<?php echo $result;?>").split("");
	$("#d0").text(numero[0]);
	$("#d1").text(numero[1]);
	$("#d2").text(numero[2]);
	$("#d3").text(numero[3]);
	...
	$("#agente").text(("<?php echo $retailer;?>"));
	$("#premio").text(('C$ ' + parseInt(("<?php echo $jackpot;?>"))).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"));
	$("#nSorteo-sor").text(pad(("<?php echo $drawnumber;?>"),5));
